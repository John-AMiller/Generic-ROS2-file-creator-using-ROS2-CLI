...
case $choice in
    ...
    5)
        ...
        ;;
    6)  echo "Creating a Message..."
        read -p "Do you want to create a launch file for your Message? (y/n)" create_launch
        ;;
    *)
...
read -p "Class constructor name (Ex: FirstNode):" constructor_name
read -p "Node name (Ex: first_node):" node_name
...
EOF
        ...
        if ! grep -q "$file_name" $setup_file; then
            sed -i "/console_scripts/a \  'service = $package_name.$file_name:main'," $setup_file
        fi
...
EOF 
        setup_file =~/ros2_ws/src/$package_name/setup.py
        if ! grep -q "$file_name" $setup_file; then
            sed -i "/console_scripts/a \  'client = $package_name.$file_name:main'," $setup_file
        fi
...
EOF
    ...
    package_file=~/ros2_ws/src/$package_name/package.xml
    if ! grep -q "<exec_depend>ros2launch</exec_depend>" $package_file; then
        sed -i "/<\/build_depend>/a \ <exec_depend>ros2launch</exec_depend>" $package_file
    fi
...
echo "Dependencies checked, workspace built and sourced"